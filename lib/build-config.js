...
async function buildConfig(opts = {}) {
  ...
  // Per-app profiles. The daemon picks one by the frontmost bundle id and falls
  // back to the tables above, so a profile only lists what it changes.
  if (found.terminal) {
    config.profiles.warp = {
      label: found.terminal.name,
      match: [found.terminal.bundle],
      bindings: {},
      l1_bindings: {},
    }
    if (found.herdr) {
      // Sits ahead of the plain terminal profile and only wakes up when herdr is
      // installed, which is what `requires` is for.
      config.profiles = {
        herdr: {
          label: 'herdr',
          requires: 'herdr',
  ...
  if (found.lightroom) {
    // Culling is the whole point: move, judge, move on, without a keyboard.
    config.profiles.lightroom = {
      label: 'Lightroom',
      match: found.lightroom.bundles,
      bindings: {
        dpad_left: { ...hotkey('left'), repeat: true },
        dpad_right: { ...hotkey('right'), repeat: true },
        south: { _note: 'Flag as a pick.', ...hotkey('p') },
        east: { _note: 'Reject.', ...hotkey('x') },
        west: { _note: 'Unflag.', ...hotkey('u') },
        north: { _note: 'Loupe view.', ...hotkey('e') },
        r1: { type: 'click', button: 'left' },
        r2: { _note: 'Compare.', ...hotkey('c') },
        l2: { _note: 'Grid.', ...hotkey('g') },
      },
      l1_bindings: {
        south: { _note: '1 star', ...hotkey('1') },
        east: { _note: '2 stars', ...hotkey('2') },
        north: { _note: '3 stars', ...hotkey('3') },
        west: { _note: '4 stars', ...hotkey('4') },
        r1: { _note: '5 stars', ...hotkey('5') },
      },
    }
  } else {
    notes.push({ what: 'Lightroom profile', why: 'Lightroom not found, so its profile was left out.' })
  }
  ...
  if (found.herdr && found.terminal) {
    ...
    config.l1_bindings = {
      ...
      west: { type: 'text', text: '/clear' },
      r1: { type: 'text', text: '/compact' },
      // The bare button walks agents; the L1 layer keeps the workspace ring, so
      // both are reachable and neither needs the configurator to get back.
